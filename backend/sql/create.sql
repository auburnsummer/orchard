...
-- level table.
-- generally speaking, anything that is directly derived from the rdzip
-- or the sources.txt is in here.
create table orchard.level (
    ...
    -- FROM THE SOURCES.TXT
    download_url        text             not null, -- if it's null, that means it's only an IPFS link
    submission_method   text             not null, --driver-generated name of the group
    human_name          text             not null, -- human-name of the group
    submission_info     jsonb            ,-- optional submission-specific data inserted by the driver.
    submission_iid      text             not null -- a submission method specific iid
);
